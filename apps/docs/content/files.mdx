...
shared, reloaded, or bookmarked. Writes use `history.replaceState` (Back leaves
the page rather than replaying keystrokes) and ride the built-in 300 ms search
debounce. A deep-linked `?folder=` id rebuilds its breadcrumb automatically.
