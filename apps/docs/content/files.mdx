...
`FileManager` persists its settled state in the query string by default —
`?search=…`, `?folder=<id>`, and `?page=…` — so a filtered library view can be
shared, reloaded, or bookmarked. Writes replace the current history entry
(Back leaves the page rather than replaying keystrokes) and ride the built-in
300 ms search debounce. A deep-linked `?folder=` id rebuilds its breadcrumb
automatically.
...
Turn it off for embedded surfaces with `urlParams={false}`, or namespace the
parameters with `urlParamPrefix="files-"` when two url-synced lists share one
page. Defaults (empty search, root folder, page 1) stay off the URL entirely.
